...
foreach ($generatedComposerJson["require"] as $name => $version) {
  if (!$version) {
    $version = "*";
  }

  // Fix invalid versions of the form 7.*-1.* generated by drush make-convert
  $version = preg_replace("/7\.\*-(\d)\.\*/", "$1.x-dev", $version);

  $generatedComposerJson["require"][$name] = str_replace("^", "", $version);
}
