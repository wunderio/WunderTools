...
if [ $FIRST_RUN ]; then
  ...
  if [ $ANSIBLE_TAGS ]; then
    ansible-playbook $EXTRA_OPTS $VAGRANT_CREDENTIALS $PLAYBOOKPATH -c ssh -i $INVENTORY -e "@$EXTRA_VARS" --ask-sudo-pass --vault-password-file=$VAULT_FILE --tags "$ANSIBLE_TAGS"
  elif [ $ANSIBLE_SKIP_TAGS ]; then
    ansible-playbook $EXTRA_OPTS $VAGRANT_CREDENTIALS $PLAYBOOKPATH -c ssh -i $INVENTORY -e "@$EXTRA_VARS" --ask-sudo-pass --vault-password-file=$VAULT_FILE --skip-tags "$ANSIBLE_SKIP_TAGS"
  else
   ansible-playbook $EXTRA_OPTS $VAGRANT_CREDENTIALS $PLAYBOOKPATH -c ssh -i $INVENTORY -e "@$EXTRA_VARS" --ask-sudo-pass --vault-password-file=$VAULT_FILE
  fi
fi
